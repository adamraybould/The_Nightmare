...
local function CreateNightmareVersion(inst, giver, pet)
    local unit = nil
    ...
    if pet == true then
        if inst.prefab == "crawlinghorror" then
            unit = giver.components.petleash:SpawnPetAt(pos.x, pos.y, pos.z, "crawlingnightmare")
        elseif inst.prefab == "terrorbeak" then
            unit = giver.components.petleash:SpawnPetAt(pos.x, pos.y, pos.z, "nightmarebeak")
        end    
    else 
        if inst.prefab == "crawlinghorror" then
            unit = SpawnPrefab("crawlingnightmare")
        elseif inst.prefab == "terrorbeak" then
            unit = SpawnPrefab("nightmarebeak")
        end
    end

    unit.sg:GoToState("appear")
    unit.Transform:SetPosition(pos:Get())
    unit:RemoveComponent("sanityaura")  
    unit:RemoveTag("shadow")
    unit:RemoveTag("nightmarecreature")
    return unit
end

local function ShouldAcceptItem(inst, item, giver)
    if item.prefab ~= "nightmarefuel" then
        giver.components.talker:Say(GetString(giver, "ANNOUNCE_SHADOWWRONGITEM"))
        return false
    end

    if giver.components.petleash:IsFull() then
        giver.components.talker:Say(GetString(giver, "ANNOUNCE_MAXPETS"))
        return false
    end

    return giver:HasTag("shadowtamer")
end

local function OnGetItemFromPlayer(inst, giver, item)
    local unit = CreateNightmareVersion(inst, giver, true)
    if unit ~= nil then
        unit.components.combat:SetTarget(nil)
    end
end
