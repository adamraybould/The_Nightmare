...
local function ShouldAttack(self)
    if self.inst.components.follower ~= nil and self.inst.components.follower.leader ~= nil then
        if self.inst.components.follower.leader.components.combat.target ~= nil and leaderTarget == nil then
            leaderTarget = self.inst.components.follower.leader.components.combat.target
        end

        if leaderTarget ~= nil then
            self.inst.components.combat:SetTarget(leaderTarget)

            if leaderTarget.components.health:IsDead() then
                leaderTarget = nil
            end
        end
    end
